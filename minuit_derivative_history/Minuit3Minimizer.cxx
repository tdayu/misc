...
// Implementation file for class Minuit3Minimizer

#include "Minuit2/Minuit3Minimizer.h"
...
Minuit3Minimizer::Minuit3Minimizer(ROOT::Minuit2::EMinimizerType type)
...
Minuit3Minimizer::Minuit3Minimizer(const char *type) : Minimizer(), fDim(0), fMinimizer(0), fMinuitFCN(0), fMinimum(0)
{
...
void Minuit3Minimizer::SetMinimizerType(ROOT::Minuit2::EMinimizerType type)
{
   // Set  minimizer algorithm type
   fUseFumili = false;
   switch (type) {
   case ROOT::Minuit2::kMigrad:
      // std::cout << "Minuit3Minimizer: minimize using MIGRAD " << std::endl;
   ...
   case ROOT::Minuit2::kSimplex:
      // std::cout << "Minuit3Minimizer: minimize using SIMPLEX " << std::endl;
...
Minuit3Minimizer::~Minuit3Minimizer()
{
...
Minuit3Minimizer::Minuit3Minimizer(const Minuit3Minimizer &) : ROOT::Math::Minimizer()
{
   // Implementation of copy constructor.
}

Minuit3Minimizer &Minuit3Minimizer::operator=(const Minuit3Minimizer &rhs)
{
...
bool Minuit3Minimizer::SetVariable(unsigned int ivar, const std::string &name, double val, double step)
{
   // set a free variable.
   ...
   MnPrint print("Minuit3Minimizer::SetVariable", PrintLevel());
...
bool Minuit3Minimizer::SetLowerLimitedVariable(unsigned int ivar, const std::string &name, double val, double step,
                                               double lower)
{
...
bool Minuit3Minimizer::SetUpperLimitedVariable(unsigned int ivar, const std::string &name, double val, double step,
                                               double upper)
{
...
bool Minuit3Minimizer::SetLimitedVariable(unsigned int ivar, const std::string &name, double val, double step,
...
bool Minuit3Minimizer::SetFixedVariable(unsigned int ivar, const std::string &name, double val)
{
...
std::string Minuit3Minimizer::VariableName(unsigned int ivar) const
{
...
int Minuit3Minimizer::VariableIndex(const std::string &name) const
{
...
bool Minuit3Minimizer::SetVariableValue(unsigned int ivar, double val)
{
...
bool Minuit3Minimizer::SetVariableValues(const double *x)
{
   // set value for variable ivar (only for existing parameters)
...
bool Minuit3Minimizer::SetVariableStepSize(unsigned int ivar, double step)
{
...
bool Minuit3Minimizer::SetVariableLowerLimit(unsigned int ivar, double lower)
{
...
bool Minuit3Minimizer::SetVariableUpperLimit(unsigned int ivar, double upper)
{
   // set the limits of an existing variable
   // parameter must exist or return false
   if (ivar >= fState.MinuitParameters().size())
...
bool Minuit3Minimizer::SetVariableLimits(unsigned int ivar, double lower, double upper)
{
   // set the limits of an existing variable
   // parameter must exist or return false
   if (ivar >= fState.MinuitParameters().size())
...
bool Minuit3Minimizer::FixVariable(unsigned int ivar)
{
...
bool Minuit3Minimizer::ReleaseVariable(unsigned int ivar)
{
...
bool Minuit3Minimizer::IsFixedVariable(unsigned int ivar) const
{
   // query if variable is fixed
   if (ivar >= fState.MinuitParameters().size()) {
      MnPrint print("Minuit3Minimizer", PrintLevel());
...
bool Minuit3Minimizer::GetVariableSettings(unsigned int ivar, ROOT::Fit::ParameterSettings &varObj) const
{
...
void Minuit3Minimizer::SetFunction(const ROOT::Math::IMultiGenFunction &func)
{
...
void Minuit3Minimizer::SetFunction(const ROOT::Math::IMultiGradFunction &func)
{
   // set function to be minimized
   fDim = func.NDim();
   if (fMinuitFCN)
...
bool Minuit3Minimizer::Minimize()
{
   // perform the minimization
   // store a copy of FunctionMinimum

   MnPrint print("Minuit3Minimizer::Minimize", PrintLevel());
   ...
   if (PrintLevel() >= 1) {
      ...
      std::cout << "Minuit3Minimizer: Minimize with max-calls " << maxfcn_used << " convergence for edm < " << tol
   ...
   if (minuit2Opt) {
      ...
      if (printLevel > 0) {
         std::cout << "Minuit3Minimizer::Minuit  - Changing default options" << std::endl;
         minuit2Opt->Print();
      }
   }
...
bool Minuit3Minimizer::ExamineMinimum(const ROOT::Minuit2::FunctionMinimum &min)
{
   ...
   MnPrint print("Minuit3Minimizer::Minimize", debugLevel);
...
void Minuit3Minimizer::PrintResults()
{
   ...
   if (fMinimum->IsValid()) {
      // valid minimum
      std::cout << "Minuit3Minimizer : Valid minimum - status = " << fStatus << std::endl;
      ...
      std::cout << "Minuit3Minimizer : Invalid Minimum - status = " << fStatus << std::endl;
      std::cout << "FVAL  = " << fState.Fval() << std::endl;
      std::cout << "Edm   = " << fState.Edm() << std::endl;
      std::cout << "Nfcn  = " << fState.NFcn() << std::endl;
   }
...
const double *Minuit3Minimizer::Errors() const
{
...
double Minuit3Minimizer::CovMatrix(unsigned int i, unsigned int j) const
{
...
bool Minuit3Minimizer::GetCovMatrix(double *cov) const
{
...
bool Minuit3Minimizer::GetHessianMatrix(double *hess) const
{
...
double Minuit3Minimizer::Correlation(unsigned int i, unsigned int j) const
{
...
double Minuit3Minimizer::GlobalCC(unsigned int i) const
{
...
bool Minuit3Minimizer::GetMinosError(unsigned int i, double &errLow, double &errUp, int runopt)
{
   ...
   MnPrint print("Minuit3Minimizer::GetMinosError", PrintLevel());
...
int Minuit3Minimizer::RunMinosError(unsigned int i, double &errLow, double &errUp, int runopt)
{
   ...
   if (runLower) {
      if (debugLevel >= 1) {
         std::cout << "************************************************************************************************"
                      "******\n";
         std::cout << "Minuit3Minimizer::GetMinosError - Run MINOS LOWER error for parameter #" << i << " : "
   ...
   if (runUpper) {
      if (debugLevel >= 1) {
         std::cout << "************************************************************************************************"
                      "******\n";
         std::cout << "Minuit3Minimizer::GetMinosError - Run MINOS UPPER error for parameter #" << i << " : "
                   << par_name << " using max-calls " << maxfcn_used << ", tolerance " << tol << std::endl;
      }
...
bool Minuit3Minimizer::Scan(unsigned int ipar, unsigned int &nstep, double *x, double *y, double xmin, double xmax)
{
   ...
   MnPrint print("Minuit3Minimizer::Scan", PrintLevel());
   if (!fMinuitFCN) {
...
bool Minuit3Minimizer::Contour(unsigned int ipar, unsigned int jpar, unsigned int &npoints, double *x, double *y)
{
   // contour plot for parameter i and j
   // need a valid FunctionMinimum otherwise exits

   MnPrint print("Minuit3Minimizer::Contour", PrintLevel());
...
bool Minuit3Minimizer::Hesse()
{
   ...
   MnPrint print("Minuit3Minimizer::Hesse", PrintLevel());
   ...
   if (fMinimum) {
      ...
      // if (PrintLevel() >= 3) {
      //    std::cout << "Minuit3Minimizer::Hesse  - State before running Hesse " << std::endl;
   ...
   if (PrintLevel() >= 3) {
      std::cout << "Minuit3Minimizer::Hesse  - State returned from Hesse " << std::endl;
      std::cout << fState << std::endl;
   }
...
int Minuit3Minimizer::CovMatrixStatus() const
{
...
void Minuit3Minimizer::SetTraceObject(MnTraceObject &obj)
{
   // set trace object
   if (!fMinimizer)
      return;
...
void Minuit3Minimizer::SetStorageLevel(int level)
{
   // set storage level
   if (!fMinimizer)
      return;
